...
export default function EditProductPage() {
    ...
    const fetchProduct = async () => {
      try {
        ...
        setFormData({
          ...
          rentable: product.rentable !== undefined ? product.rentable : true,
          status: product.status || 'available'
        });
      } catch (err) {
        setError('Failed to fetch existing product details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.put(`/products/update-product/${id}`, formData);
  ...
  if (loading)
    return <div className="animate-pulse p-8">Loading product data...</div>;
      ...
      <div className="p-8">
        <div className="mb-8">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Update details for product ID: {id}
          </p>
        </div>
